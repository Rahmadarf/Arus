...
export default function NavigationShell({ children, userProfile, finances }: NavShellProps) {
  ...
  return (
    <div className="flex min-h-screen bg-zinc-50 dark:bg-zinc-950">
      ...
      <nav className="md:hidden fixed bottom-0 inset-x-0 bg-white border-t border-zinc-200 dark:bg-zinc-900 dark:border-zinc-800 z-40 flex justify-around items-center h-16 px-2 safe-area-bottom pb-env">
        {primaryNavItems.map(({ href, label, icon: Icon }) => (
          <Link key={href} href={href} className="flex flex-col items-center justify-center w-full h-full space-y-1 text-zinc-500 hover:text-zinc-900 dark:text-zinc-400 dark:hover:text-zinc-50">
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden fixed inset-0 z-50 flex flex-col justify-end">
          ...
          <div className="relative w-full bg-white dark:bg-zinc-900 rounded-t-2xl p-5 shadow-xl animate-in slide-in-from-bottom-full duration-200">
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center space-x-3">
                ...
                <span className="text-sm font-semibold text-zinc-900 dark:text-zinc-50">{userProfile.fullName}</span>
              </div>
              <button onClick={() => setIsMobileMenuOpen(false)} className="p-2 text-zinc-500 bg-zinc-100 rounded-full dark:bg-zinc-800">
                <X className="h-4 w-4" />
              </button>
            </div>
            ...
            <div className="space-y-4">
              {secondaryNavItems.map(({ href, label, icon: Icon }) => (
                <Link key={href} href={href} onClick={() => setIsMobileMenuOpen(false)} className="flex items-center gap-3 p-3 rounded-xl bg-zinc-50 text-sm font-medium text-zinc-700 dark:bg-zinc-800 dark:text-zinc-300">
                  <Icon className="h-5 w-5 text-zinc-400" /> {label}
                </Link>
              ))}
