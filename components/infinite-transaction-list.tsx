...
// 1. KOREKSI: Tambahkan useOptimistic di sini
...
export default function InfiniteTransactionList({ initialTransactions, type }: Props) {
  ...
  // 2. KOREKSI: Ubah nama fungsi dispatch menjadi optimisticDelete
  const [optimisticTransactions, optimisticDelete] = useOptimistic(
    transactions,
  ...
  // 3. KOREKSI: Gunakan optimisticTransactions untuk pengecekan kosong
          ...
          {optimisticTransactions.map((tx) => (
              <div key={tx.id} className="flex items-center justify-between py-3.5">
                  ...
                  <div className="flex items-center">
                      ...
                      {/* 4. KOREKSI FATAL: Suntikkan onOptimisticDelete ke komponen anak */}
                      <TransactionActions 
                        id={tx.id} 
                        type={type} 
